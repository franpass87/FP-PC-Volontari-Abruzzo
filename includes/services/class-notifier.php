...
class PCV_Notifier {
    ...
    public function build_notification_message( array $record ) {
        $lines = [
            ...
            sprintf( '%s: %s', __( 'Partecipa', self::TEXT_DOMAIN ), (int) $record['partecipa'] === 1 ? __( 'Sì', self::TEXT_DOMAIN ) : __( 'No', self::TEXT_DOMAIN ) ),
            sprintf( '%s: %s', __( 'Pernotta', self::TEXT_DOMAIN ), (int) $record['dorme'] === 1 ? __( 'Sì', self::TEXT_DOMAIN ) : __( 'No', self::TEXT_DOMAIN ) ),
            sprintf( '%s: %s', __( 'Pasti', self::TEXT_DOMAIN ), (int) $record['mangia'] === 1 ? __( 'Sì', self::TEXT_DOMAIN ) : __( 'No', self::TEXT_DOMAIN ) ),
            '',
